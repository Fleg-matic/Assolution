-- Script d'initialisation de la base de données Assolution
...
BEGIN
   IF NOT EXISTS (
      ...
      CREATE ROLE assolution_user LOGIN PASSWORD 'Assolution2025!';
   END IF;
END
$do$;
...
-- Table des utilisateurs (exemple - sera créée par Hibernate)
-- Cette table sera automatiquement créée par Spring Boot/Hibernate
-- mais on peut définir quelques contraintes de base
...
-- Insérer quelques données de test pour le développement
-- Ces données ne seront présentes que dans la base de développement

-- Utilisateur administrateur de test (mot de passe: AdminTest123!)
-- Le hash sera généré par l'application Spring Boot
